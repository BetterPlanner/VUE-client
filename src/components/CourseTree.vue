<template>
  <v-card>
        ...
        <v-card v-else :key="1" class="pt-6 mx-auto" flat max-width="900">
          <v-card-text v-if="selected_data">
            <h3 class="headline mb-2">
              <a :href="`/view?course=${selected_data.code}`" target="_blank"
                >{{ selected_data.code }} - {{ selected_data.name }}</a
              >
            </h3>
          </v-card-text>
          <v-divider></v-divider>
          <v-row v-if="selected_data" class="text-left" tag="v-card-text">
            <v-col class="text-right mr-4 mb-4" tag="strong" cols="2"
              >Description:</v-col
            >
          ...
          <v-row class="text-left" tag="v-card-text" v-if="selected_data">
            <v-col class="text-right mr-4 mb-2" tag="strong" cols="2"
              >Campus:</v-col
            >
    ...
    <!-- <v-row>
      <v-col class="d-flex text-center">
        <v-card :key="1" class="pt-6 mx-auto" flat>
          <div v-for="course in getCourse" :key="course.name">
            {{ course.name + " " + course.code }}
          </div>
          <ApolloQuery
            v-if="search"
            :query="require('../graphql/getCourse.gql')"
            :variables="{ search }"
          >
            <template v-slot="{ result: { loading, error, data } }">
              <div v-if="data">
                <div v-for="course in data.getCourse" :key="course.code">
          ...
          <div>
            <div v-for="course in getCourse" :key="course.code">
              {{ "Name: " + course.name + " Code: " + course.code }}
            </div>
          </div>
        </v-card>
...
export default {
  ...
  watch: {
    ...
    search(val) {
      ...
      axios
        .get(`http://localhost:8080/api/search_queries?query=` + val)
        .then(response => {
  ...
  methods: {
    ...
    onUpdate(selection) {
      ...
      axios
        .get(`http://localhost:8080/api/search?course=` + selection)
        .then(response => {
